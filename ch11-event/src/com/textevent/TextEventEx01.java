...
public class TextEventEx01 extends Frame implements ActionListener, TextListener{	
	...
	public void actionPerformed(ActionEvent e){
		//String getActionCommand(): Action을 발생시킨 객체의 문자열을 얻어옵니다.
		if( e.getActionCommand().equals("입력")){
			//TextField의 내용 읽어와서 TextArea에 추가
	...
	public void textValueChanged(TextEvent e){
		...
		if(tf.getText().equals("")){ //입력한 데이터가 없는 경우
			//void setEnabled(Boolean b): 파라미터 b값에 의해 컴포넌트의 활성화(true)와 비활성화(false)를 지정합니다.
			input.setEnabled(false);					
		}else{ //입력한 데이터가 있는 경우
