...
JDK5.0 이전에는 특정 메서드를 정의할 때 인자(argument)의 타입과 수를 정해 두고 호출 시 
전달되는 인자(argument)의 수가 일치하지 않을 경우에는 메서드를 호출할 수가 없었습니다(Error)
이런 문제를 해결하기 위해 메서드의 이름은 같지만 인자의 수가 다른 여러 개의 메서드를 정의하는 메서드오버로딩(Overloading) 
또는 메서드 정의 시에 배열객체를 인자로 지정하여 줍니다.
