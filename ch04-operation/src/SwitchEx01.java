...
[switch 형식]
switch문 전체가 실행되는 것이며 
인자값에 해당되는 것이 있으면 해당 레이블로 이동해서 실행되고
 레이블에 해당되는 것이 없으면 default레이블 영역에 있는 구문이 실행 됩니다.
...
case 조건값3 :         
        수행문; 
    break      
default : 	// 받은 인자값이 case문의 조건값1에서 조건값3까지 일치하는 것이 단 하나도 없다면 
			//default레이블의 수행문을 처리합니다.
        수행문;            
 
}
