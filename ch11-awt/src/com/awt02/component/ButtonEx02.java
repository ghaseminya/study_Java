...
패널을 사용해 Button 크기 지정 (ButtonEx03.java)  

기본 Frame 사용시
	• setSize로 지정한 버튼 크기로 표지되지 않고 Frame크기로 확대되는 현상 발생
	• 레이아웃을 사용해 버튼의 크기를 강제해야 하는데 레이아웃을 사용하지 않아서 버튼이 확대되는 현상 발생
	• 컴포넌트는 기본적으로 setSize가 안먹혀있어서 프레임 크기로 확대된 것입니다.  (setLayout 기본 적용되어 있음)
	• Frame 컨테이너는 기본 배치관리자로 BorderLayout가 기본 적용되어 있습니다.


해결방법:
1)setSize를 사용하고 setLayout을 사용안할려면 setLayout 사용을 안한다고 지정해 주어야 합니다.
...
public class ButtonEx02 extends Frame{
	...
	public ButtonEx02(){
		...
		//Button 사이즈 지정
		// setSize로 지정한 버튼 크기로 표지되지 않고 전체 Frame크기로 확대되는 현상 발생
