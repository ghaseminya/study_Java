...
FileReaderWriter.txt
한글, 영문으로 된 문자열이 있는 파일의 내용을 만들어서 테스트 (소스코드 내용도 가능)
...
public class FileReaderWriterEx01 {
	...
	public static void main(String[] args){
		...
		try{
			...
			//스트림으로부터 0인 시작 위치부터100바이트만큼 읽어 buffer 배열에 저장합니다.
			fReader.read(buffer, 0, buffer.length);
			
			//스트림으로부터 0인 시작 위치부터 char배열 buffer의 길이만큼 읽어 buffer 배열에 저장합니다.
//			fReader.read(buffer, 0, buffer.length);
			
			//buffer 배열에 있는 데이터를 문자열로 변환
