...
[Inheritance(상속)]
	...
	• 특정 클래스(super class)가 가지는 일부 속성과 기능을 다른 새로운 클래스(sub class)에게 제공하기 위해 맺는 클래스들간의 관계
	• extends 예약어를 사용
	• 멤버변수, 멤버메소드는 상속되지만
	• 생성자는 상속이 안됩니다.
...
[상속관계 용어 정리 (동일 용어)]
구분									동일용어
• 상속을하기 위해 준비된 특정 클래스,				• Base Class(기본 클래스, 기초클래스, 기반클래스)
물려 주는 클래스,							• Super Class(상위클래스, 슈퍼 클래스)
상속을 해주는 쪽							• Parent Class(부모 클래스)

• 특정 클래스로부터 상속을 받아 새롭게 정의되는 클래스,	• Derivation Class(유도 클래스, 파생클래스)
물려 받는 클래스,							• Sub Class(하위 클래스)
상속을 받는 쪽							• Child Class(자식 클래스)
