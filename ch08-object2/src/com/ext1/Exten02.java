...
[상속 특징]
IS-A 관계						• 객체간에 IS-A 관계가 성립되어야 합니다.

단일 상속(Single Inheritance)	• 자바는 단일 상속(Single Inheritance)만 지원합니다
							• (부모 클래스는 여러 자식 클래스를 가질 수 있지만 자식 클래스는 하나의 부모 클래스만 가질 수 있습니다)
							
부모 클래스를 먼저 객체 생성		• 클래스 객체가 생성될 때 자동으로 부모 클래스를 먼저 객체 생성 한 후에 자신의 클래스가 생성됩니다.
